...
function BlogDetails() {
  ...
  var formattedDateNY = new Date(post.date).toLocaleDateString("ar-EG", {
    day: "numeric",
    month: "long",
  });

  const contentParts = post.content.split("##");
  const preview = contentParts[0].trim();
  const sectionParts = contentParts.slice(1);

  const sections = [];
  for (let i = 0; i < sectionParts.length; i++) {
    const lines = sectionParts[i].trim().split("\n\n");
